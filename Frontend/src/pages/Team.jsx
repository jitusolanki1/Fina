...
function Team() {
  ...
  const handleAdd = useCallback(
    async (e) => {
      ...
      try {
        ...
        setShowAdd(false);
        setForm({ name: "", email: "", teams: "", role: "Member" });
        load();
      } catch (err) {
        console.error(err);
  ...
  return (
    <div>
      {forbidden && (
        <div className="table-card mb-4 p-6">
          <div className="text-sm text-slate-300">
            You do not have permission to view team members. Contact an
            administrator.
          </div>
        </div>
      )}
      <div className="table-card mb-4">
        ...
        <div className="table-footer">
          <div>
            {total === 0 ? 0 : (page - 1) * per + 1} -{" "}
            {Math.min(page * per, total)} of {total} users
          </div>
          <div className="pager">
            <button
              className="page-btn"
            ...
            <form onSubmit={handleAdd} className="space-y-3">
              ...
              <div>
                ...
                <input
                  value={form.teams}
                  onChange={(e) =>
                    setForm((f) => ({ ...f, teams: e.target.value }))
                  }
                  className="p-2 w-full rounded bg-[#0A0A0A] border border-[var(--border)] text-white"
                />
              </div>
